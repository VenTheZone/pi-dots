...
This project is a pi-coding-agent compatible fork of `favorite-opencode-setup`.
...
- plugins
- provider/model configuration from the original harness
- source-harness-specific runtime wiring that is now replaced by local pi packages in this repo
...
# Imported Workflow Rules for Pi

This document consolidates the core rules and guidelines from the original setup for use with pi-coding-agent.
...
Pi keeps the core harness minimal, so the following actions that may have been automated in other tools should be done manually unless you install or build extensions for them:
